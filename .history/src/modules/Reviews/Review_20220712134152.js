...
const Review = () => {
  ...
  const demo1 = likeData.filter((item) => item[0]["0"]?.userId === userInfo.id);
  console.log("🚀 ~ file: Review.js ~ line 72 ~ Review ~ demo1", demo1);

  useEffect(() => {
    if (demo1 && toggleLike === true && demo1.like === 1) return;
    ...
    addLikeDB();
  }, [data, like, demo1, toggleLike]);

  // useEffect(() => {
  //   const addLikeDB = async () => {
  //     if (like !== 0 && toggleLike === false) {
