...
const Review = () => {
  ...
  console.log(
    "🚀 ~ file: Review.js ~ line 47 ~ Review ~ toggleLike",
    toggleLike
  );
  ...
  const idLike = likeData
    .map((item) => item[0]?.["0"]?.userId === userInfo.id)
    .map((item) => item.id);
  console.log("🚀 ~ file: Review.js ~ line 68 ~ Review ~ idLike", idLike);
  ...
  const handleClickLike = async () => {
    ...
    const addLikeDB = async () => {
      let colRef = collection(db, "like", idLike);
      await addDoc(colRef, {
        ...data,
        like: 1,
      });
