...
const Review = () => {
  ...
  const demo1 = likeData.filter(
    (item) => item[0]?.["0"]?.userId === userInfo.uid
  );

  const demo2 = likeData.map((item) => item[0]?.["0"]?.userId === userInfo.uid);
  // .map((item) => item.id);
  console.log("🚀 ~ file: Review.js ~ line 66 ~ Review ~ demo2", demo2);
    ...
    if (demo1 && toggleLike === false) {
      ...
      const addLikeDB = () => {
        demo2.forEach(async (idLike) => {
          console.log(idLike);
          let colRef = doc(db, "like", idLike[0]);
